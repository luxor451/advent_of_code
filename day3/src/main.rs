...
fn find_max_and_index(input: Vec<char>) -> (char, usize) {
    let mut max = input[0].to_digit(10).unwrap();
    let mut max_index = 0;
    ...
    (input[max_index], max_index)
}

fn find_max_per_banks(nb_of_battery: usize, bank: Vec<char>) -> u64 {
    let mut last_index = 0;
    let mut char_vec_of_number: Vec<char> = Vec::with_capacity(12);
    for i in (0..nb_of_battery).rev() {
        let elem_to_shearch = &bank[last_index..bank.len() - i];
        let (max, max_index) = find_max_and_index(elem_to_shearch.to_vec());
        last_index = (max_index + last_index) + 1;

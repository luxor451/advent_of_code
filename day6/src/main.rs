...
fn func(content : &Vec<Vec<char>>, j : usize, m: usize, n : usize, elem_index : usize) -> u64{
    ...
    let current_nb_vec: Vec<char> = current_nb_vec.into_iter().filter(|x| *x != ' ').collect::<Vec<char>>();
    let current_number: u64 = current_nb_vec.iter().collect::<String>().parse::<u64>().unwrap();
